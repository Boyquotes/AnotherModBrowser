...
def markdown_to_bbcode(s):
    ...
    s = re.sub(r"(?m)^#\s+(.*?)\s*#*$", translate("~[font_size=24][b]%s[/b][/font_size]"), s)
    s = re.sub(r"(?m)^##\s+(.*?)\s*#*$", translate("~[font_size=22][b]%s[/b][/font_size]"), s)
    s = re.sub(r"(?m)^###\s+(.*?)\s*#*$", translate("~[font_size=20][b]%s[/b][/font_size]"), s)
...
if __name__ == '__main__':
    print(markdown_to_bbcode(sys.argv[1]))
